...
namespace Practice_Linq
{
    public class Program
    {
        ...
        static void Query1(List<FootballGame> games)
        {
            ...
            {
                Console.WriteLine($"{g.Date:yyyy-MM-dd} {g.Home_team} - {g.Away_team} ({g.Home_score}:{g.Away_score}) - {g.City}");
            }
        }
        ...
        static void Query2(List<FootballGame> games)
        {
            ...
            foreach (var g in selectedGames)
            {
                Console.WriteLine($"{g.Date:yyyy-MM-dd} {g.Home_team} - {g.Away_team} ({g.Home_score}:{g.Away_score})");
            }
        ...
        static void Query7(List<FootballGame> games)
        {
            ...
            {
                Console.WriteLine("Матчів не знайдено");
            }
        }
        ...
        static void Query8(List<FootballGame> games)
        {
            ...
            var selectedGames = games
                .Where(g => g.Tournament == "UEFA Euro" && g.Date.Year == 2012 && g.Country == "Ukraine")
                .Select(g => new
                {
            ...
            foreach (var g in selectedGames)
            {
                Console.WriteLine($"{g.MatchYear} {g.Team1} - {g.Team2}, Goals: {g.Goals}");
            }
        ...
        static void Query9(List<FootballGame> games)
        {
            ...
            foreach (var g in selectedGames)
            {
                Console.WriteLine($"{g.MatchYear} {g.Game}, Result for {g.Game.Split('-')[0]}: {g.Result}");
            }
        ...
        static void Query11(List<FootballGame> games)
        {
            ...
            //foreach
            foreach (var country in selectedGames)
            {
        ...
        static void Query12(List<FootballGame> games)
        {
            ...
            {
                Console.WriteLine($"{t.Tournament}: {t.Count}");
            }
        ...
        static void Query15(List<FootballGame> games)
        {
            ...
            var allTeams = games.Select(g => g.Home_team)
                               .Concat(games.Select(g => g.Away_team))
                               .Distinct();

            var selectedGames = allTeams
                .Select(team => new
                {
                    Team = team,
                    Count = games.Count(g => g.Home_team == team || g.Away_team == team)
                })
                .Where(t => t.Count == 1)
                .OrderBy(t => t.Team)
                .ToList();
            ...
            foreach (var t in selectedGames)
            {
                Console.WriteLine($"{t.Team}: {t.Count}");
            }
